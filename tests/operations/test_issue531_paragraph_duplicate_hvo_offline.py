#
#   test_issue531_paragraph_duplicate_hvo_offline.py
#
#   Offline ratchet for issue #531 Duplicate insert_after HVO index lookup.
#
#   Copyright 2026
#
...
def test_issue531_duplicate_insert_after_uses_hvo_index():
    block = _duplicate_block(PARAGRAPH_OPS.read_text(encoding="utf-8"))
    assert "issue #531" in block
    assert "p.Hvo == target_hvo" in block
    assert "para_list.index(para_obj)" not in block


def test_issue531_live_gate_module_exists():
    assert LIVE_GATE.is_file(), "missing live gate module for #531"
