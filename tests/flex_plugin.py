...
# flex_plugin.py
#
# Pytest plugin providing FieldWorks initialization and shared fixtures
# for the whole test suite.
#
# This lives in a plugin module rather than in tests/conftest.py because a
# conftest.py applies only to its own directory subtree. flexicon/tests/
# and flexicon/sync/tests/ sit outside tests/, so a file- or
# directory-targeted run in either one picked up no FLEx initialization and
# none of the evidence-writing hooks below (issue #307). The repository-root
# conftest.py registers this module as a plugin, which makes every fixture
# and hook here global to all invocation shapes.
#
# Provides:
...
def pytest_configure(config):
    """Set up sys.path and register custom markers for test categorization."""
    # Re-assert the package root. The module-level insert above covers the
    # normal path; this covers a plugin loaded before that ran.
    if _project_root not in sys.path:
        sys.path.insert(0, _project_root)
