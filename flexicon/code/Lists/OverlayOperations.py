...
class OverlayOperations(PossibilityItemOperations):
    ...
    def FindByChart(self, chart):
        ...
        Notes:
            - ``chart`` is validated but not used for lookup; overlays are
              project-scoped at ``ILangProject.OverlaysOC`` (issue #303)
            - Equivalent to ``GetAll()`` for every chart
    ...
    def GetVisibleOverlays(self, chart):
        ...
        Returns:
            list: Visible ``ICmOverlay`` objects on the project.
        ...
        Example:
            ...
            >>> print(f"Project has {len(overlays)} visible overlays")
            Project has 2 visible overlays

        Notes:
            - ``chart`` is validated but not used for lookup (issue #303)
            - Filters ``GetAll()`` with ``IsVisible``; empty when all hidden
